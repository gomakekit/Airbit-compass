...
function compassCorrector () {
    compassDesired = 0
    compassCorrectionFactor = 0.2
    compassMeasured = input.compassHeading()
    if (compassMeasured >= 180) {
        compassMeasured = input.compassHeading() - 360
    }
    compassAverage = Math.round(compassMeasured * 0.02 + compassAverage * 0.98)
    compassCorrection = compassDesired - compassAverage
    yaw = Math.round(compassCorrection * compassCorrectionFactor)
    yaw = Math.constrain(yaw, -15, 15)
}
let buzzer = 0
let compassCorrection = 0
let compassAverage = 0
let compassMeasured = 0
let compassCorrectionFactor = 0
let compassDesired = 0
let arm = 0
let roll = 0
let pitch = 0
let yaw = 0
...
basic.forever(function () {
    ...
    radio.sendValue("C", compassAverage)
    radio.sendValue("YC", yaw)
})
